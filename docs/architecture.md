...
- Stepping is manual: `scene.physicsEnabled` is off, `World.updatePhysics` calls `physicsWorld.step(timeStep)` once per frame with the Time_Scale-scaled delta; an accumulator turns it into fixed 1/60 s substeps (max 10 per frame, like cannon's `World.step`). Pre/post-step listeners (the raycast vehicles) run per substep; the character physics bridge runs once per frame from `updatePhysics`, as upstream did.
...
- Babylon `Engine` (WebGL2 required) on a right-handed `Scene` (`useRightHandedSystem = true`, so glTF loads unflipped and three's coordinate conventions carry over), hardware scaling level `1 / Math.min(window.devicePixelRatio, 2)`.
- Colour pipeline: the scene's image processing is **off** and the frame is linear colour from end to end. That mirrors the three.js build, quirk included - with FXAA on (the default) three rendered into the composer's linear HalfFloat target and the FXAA pass wrote those raw values to the canvas (no tone mapping, no sRGB encode, clipped highlights, the saturated cyan sky every map was tuned against); only its direct-to-canvas path (FXAA off) ran ACES + sRGB, which `ToneMappingPostProcess` (`src/ts/world/ToneMappingPostProcess.ts`, ACES at exposure 1 with three's 1 / 0.6 pre-scale + LinearTosRGB) reproduces when `tickRenderPipeline` swaps it in for FXAA. Both passes run on half-float targets so values above 1 only clip at the canvas. Consequences for everything that feeds the frame: glTF textures stay sRGB buffers (the loader default, decoded to linear on sampling), colour textures three tagged `SRGBColorSpace` load through `Utils.loadColorTexture`, hex colour constants go through `Utils.linearColorFromHex` (three's `Color(0x...)` decode), glTF factors are used as the linear values they are, and the ocean's PBR material carries its own `ImageProcessingConfiguration` with `applyByPostProcess` so it writes linear too.
- Lights carry three's numbers: sun `2.5 / pi` and hemisphere `0.3-0.9 / pi` (StandardMaterial's lambert has no 1 / pi; the HSL tints are three's linear values). PBR's BRDF has the 1 / pi built in, and three's CSM ran one `DirectionalLight` per cascade that hit every material not registered with it - the ocean - all at once, so `Sky.lightAsPbr(mesh)` moves a mesh onto a second sun at `3 x 2.5`.
...
- `OutlineEffect` (in `src/ts/world/OutlineEffect.ts`) keeps a `DepthRenderer` alive while outlines are on (its render list skips meshes flagged with `markOutlineSkip` - sky / stars / earth / moon / grass / ocean) and feeds the depth texture to a Sobel-edge `PostProcess` with a scale-invariant ratio threshold; the outline colour (three's `0x222222`, decoded to linear) is mixed in like three's alpha-blended quad.
- Sky: `Sky` renders three's Sky shader (`src/ts/world/SkyShader.ts` - Preetham scattering plus the procedural cloud layer, three's uniform names) on a `ShaderMaterial`, raw linear like every other material.
...
- Materials: `Utils.setupMeshProperties` converts every loaded PBR material into a black-specular `StandardMaterial` (the lambert-ish look three's `MeshPhongMaterial` swap gave), keeping albedo texture, colour and side orientation; primitives without a glTF material and fully metallic untextured ones (the test worlds' placeholders) get a 0.5 grey lambert - three's rough white metal default has no diffuse term and came out as a mid grey under the sun. glTF loads with `createInstances: false` so nodes sharing a mesh (world.glb's collision markers) are independent meshes like in three.

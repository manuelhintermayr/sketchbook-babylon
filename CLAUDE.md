...
- `src/ts/world/` (root) - visual environment entities + small subsystems: `Sky`/`SkyShader` (three's Sky shader), `Ocean`, `Grass`/`GrassShader`/`Perlin`, `OutlineEffect`, `ToneMappingPostProcess` (three's ACES + sRGB, attached only while FXAA is off), `RaceCheckpoint`/`RaceContent`, `ProximityPrompt`.
...
- Don't switch `scene.imageProcessingConfiguration` back on, and don't hand materials gamma-space colours: the frame is deliberately linear end to end (three's composer path - see the colour pipeline note in `docs/architecture.md`). Hex constants go through `Utils.linearColorFromHex`, colour textures three tagged sRGB through `Utils.loadColorTexture`.
